  ...
  const svgElement = (
    ...
    >
      ...
      {subplot.elements.map((el, elIdx) => {
        ...
        if (el.type === "edgeDistribution") {
          const ed = el as any;
          return (
            <g key={`edgeDist-${elIdx}`}
              style={visible ? { animation: `fp-areaFade 0.8s ease 1.5s both` } : { opacity: 0 }}>
              {ed.bars.map((bar: any, bi: number) => (
                <rect
                  key={`ed-bar-${bi}`}
                  x={bar.x.toFixed(1)} y={bar.y.toFixed(1)}
                  width={bar.width.toFixed(1)} height={bar.height.toFixed(1)}
                  fill={ed.color} opacity={ed.opacity}
                  rx="1"
                />
              ))}
              {ed.annotations.map((ann: any, ai: number) => (
                <g key={`ed-ann-${ai}`}>
                  <line
                    x1={ed.areaX - 4} y1={ann.y.toFixed(1)}
                    x2={ed.areaX + 16} y2={ann.y.toFixed(1)}
                    stroke={ann.color} strokeWidth={0.8} strokeDasharray="3 2"
                  />
                  <path
                    d={`M${(ed.areaX - 2).toFixed(1)},${(ann.y - 3).toFixed(1)} l3,3 l3,-3 l-3,-3 z`}
                    fill={ann.color}
                  />
                  <text
                    x={(ed.areaX + ed.areaWidth + 4).toFixed(1)}
                    y={(ann.y + 3).toFixed(1)}
